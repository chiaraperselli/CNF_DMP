...
# Codice unico che:
# 1. carica un modello CNF allenato,
# 2. genera nuovi pesi DMP da contesti non visti (context_binary.csv),
# 3. converte in traiettorie DMP (solo x,y) con start e goal fissi
...
def generate_trajectories_from_context(context_str, model_path, samples_per_context):
    ...
    weights_df.to_csv(f"generated_weights_final_{context_str}.csv", index=False, header=False)
